...
<body>
    ...
    try {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty(trim($_POST["search_name"]))) {
            ...
            $stmt = $conn->prepare("SELECT * FROM evm.attendee WHERE att_name LIKE :search_name");
            $stmt->execute(['search_name' => "%$search_name%"]);
        } else {
            $stmt = $conn->prepare("SELECT * FROM evm.attendee");
            $stmt->execute();
        }
        ...
        // Display attendees
        echo "<table border='1'>";
        echo "<tr><th>Name</th><th>Email</th><th>Email</th><th>Address</th></tr>";
        foreach ($result as $row) {
            echo "<tr>";
            echo "<td>" . htmlspecialchars($row['att_name']) . "</td>";
